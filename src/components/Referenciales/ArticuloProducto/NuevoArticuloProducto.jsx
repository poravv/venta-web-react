...
function NuevoArticuloProducto({ token }) {
                ...
                if (articulo?.estado !== 'error') {
                    createProductoFinal({
                        ...
                        if (producto?.estado !== 'error') {
                            createReceta({
                                token, json: {
                                    idproducto: articulo.body.idproducto,
                                    receta_estado: 'AC',
                                    idproducto_final: producto.body.idproducto_final,
                                    estado: 'AC',
                                    cantidad: 1,
                                }
                            }).then((receta) => {
                                console.log(receta)
                                if (receta?.estado !== 'error') {
                                    message.success(articulo?.mensaje);
                                    message.success(producto?.mensaje);
                                    message.success(receta?.mensaje);
                                    navigate('/articulo');
                                } else {
                                    message.error(receta?.mensaje);
                                }
                            });
                        } else {
                            message.error(producto?.mensaje);
                        }
                    })
                } else {
                    message.error(articulo?.mensaje);
                }
            });
    ...
    return (
        <div >
            <div style={{ marginBottom: `20px` }}>
                <Titulos text={`NUEVO ARTICULO PRODUCTO`} level={3}></Titulos>
            </div>
            <Form
                name="basic"
                form={form}
...
export default NuevoArticuloProducto;
