...
const baseURL = process.env.REACT_APP_API_URL+'/cliente';
...
export const getVwCliente = async ({token}) => {
    //CONFIGURACION DE TOKEN
    const config = {
        headers: {
            "Authorization": `Bearer ${token}`,
...
export const getLikeCliente = async ({token,param}) => {
    //CONFIGURACION DE TOKEN
    const config = {
        headers: {
            "Authorization": `Bearer ${token}`,
    ...
    //const { data } = await axios.get(baseURL, credentials);
    const { data } = await axios.get(`${baseURL}/likeCliente/${param}`, config)
    return data;
};
...
export const updateCliente  = async ({token,param,json}) => {
    const config = {
        headers: {
            "Authorization": `Bearer ${token}`,
        }
